...
export default function SalesFees(props: SalesFeesProps) {
  ...
  const salesFees: SalesFeeItem[] = React.useMemo(() => {
    return props.sales
      .filter(sale => sale.managerName && sale.managerName !== '배정 대기')
      .map(sale => {
        const matchingManager = salesManagersList.find(m => m.name === sale.managerName);
        const inquiryType = sale.inquiryType || 'corporate';
        const rate = inquiryType === 'corporate' ? 10 : 20;
        const baseAmount = (sale.amount || 0) / 1.1;
        const calculatedFee = Math.round(baseAmount * (rate / 100));
  ...
  const handleAddFee = async (e: React.FormEvent) => {
    ...
    const baseAmount = salesAmt / 1.1;
    const calculatedFeeValue = Math.round(baseAmount * (rate / 100));
  ...
  const handleToggleStatus = async (item: SalesFeeItem) => {
    ...
    try {
      ...
      showToast(`${item.managerName} 님의 영업 수수료를 ${nextStatus === 'completed' ? '지급 완료(완결)' : '정산 대기'} 상태로 갱신 완료했습니다.`);
    } catch (err) {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto font-sans text-slate-800 pb-10 relative" id="sales_fees_wrapper">
      ...
      {/* Stats KPI Block */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6" id="sales_commission_kpis">
        ...
        <div className="lg:col-span-7 space-y-4">
          <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-xs space-y-5">
            ...
            <div className="space-y-3 max-h-[500px] overflow-y-auto pr-1">
              {activeManagerFees.length > 0 ? (
                activeManagerFees.map((fee) => (
                  <div
                    key={fee.id}
                  ...
                  >
                    <div>
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-600 border border-amber-500/20 font-mono">{fee.salesId || '매출매칭'}</span>
                        <button
                          onClick={() => handleToggleInquiryType(fee.id, (fee as any).inquiryType || 'corporate')}
                          title="클릭하여 문의 유형 변경 (개인문의 20% ↔ 회사문의 10%)"
                          className={`text-[10px] font-extrabold px-1.5 py-0.5 rounded border cursor-pointer transition-all ${
                            ((fee as any).inquiryType || 'corporate') === 'corporate'
                              ? 'bg-blue-50/80 text-blue-750 border-blue-200 hover:bg-blue-100'
                              : 'bg-emerald-50/80 text-emerald-750 border-emerald-200 hover:bg-emerald-100'
                          }`}
                        >
                          {((fee as any).inquiryType || 'corporate') === 'corporate' ? '🏢 회사문의 (10%)' : '👤 개인문의 (20%)'}
                        </button>
                      </div>
                      <h4 className="font-black text-slate-950 text-sm mt-1">{fee.managerName}  <span className="text-xs font-normal text-slate-500">→ 계약고객 성명: {fee.customerName}</span></h4>
                      <div className="mt-1 flex items-center space-x-4 text-slate-500 font-medium">
                        <span>체결 영업매출: <strong className="font-mono text-slate-705 font-black">{formatKrw(fee.salesAmount)}</strong></span>
                        <span>배정 커미션: <strong className="font-mono text-amber-600 font-bold">{fee.commissionRate}%</strong></span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between sm:justify-end gap-3 border-t border-slate-200/50 sm:border-0 pt-2 sm:pt-0">
                      <div className="text-right">
                        <span className="text-slate-400 block text-[10px]">지급 지정 수당</span>
                        <strong className="text-slate-905 font-mono font-bold text-sm block">{formatKrw(fee.calculatedFee)}</strong>
                      </div>
                ...
                <div className="py-20 text-center text-slate-400 font-sans border border-dashed border-slate-200 rounded-2xl bg-slate-50">
                  ...
                  등록된 영업 지출 수계 청구 명세가 원장에 기록에 존재하지 않습니다.
                </div>
              )}
            </div>
          </div>
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-white rounded-3xl p-6 shadow-2xl relative w-full max-w-md z-10 border border-slate-200">
              ...
              <form onSubmit={handleAddFee} className="space-y-4 text-xs font-sans">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="block text-slate-500 font-bold mb-1">문의 유형 구분 *</label>
                    <select
